...
runcmd:
  ...
  - [sh, -c, "if [ '$node' = r1 ]; then vtysh -c 'conf t' -c 'ip route $prefix blackhole' -c 'router bgp $asn' -c 'address-family ipv4 unicast' -c 'network $prefix'; sleep 45; vtysh -c 'conf t' -c 'router bgp $asn' -c 'address-family ipv4 unicast' -c 'no network $prefix' -c 'exit-address-family' -c 'no ip route $prefix blackhole' >> /var/log/danos-frr-route-cycle.log 2>&1; sleep 15; vtysh -c 'conf t' -c 'ip route $prefix blackhole' -c 'router bgp $asn' -c 'address-family ipv4 unicast' -c 'network $prefix' >> /var/log/danos-frr-route-cycle.log 2>&1; fi"]
