...
runcmd:
  ...
  - [sh, -c, "if [ '$node' = r2 ]; then sleep 80; echo '--- BGP DYNAMIC ROUTE CYCLE ---' > /dev/ttyS0; vtysh -c 'conf t' -c 'router bgp 65002' -c 'address-family ipv4 unicast' -c 'network 10.20.0.0/24' > /dev/ttyS0 2>&1; echo bgp-add-rc=\$? > /dev/ttyS0; vtysh -c 'show bgp ipv4 unicast 10.20.0.0/24' > /dev/ttyS0 2>&1 || true; sleep 20; vtysh -c 'conf t' -c 'router bgp 65002' -c 'address-family ipv4 unicast' -c 'no network 10.20.0.0/24' -c 'exit-address-family' > /dev/ttyS0 2>&1; echo bgp-withdraw-rc=\$? > /dev/ttyS0; fi"]
