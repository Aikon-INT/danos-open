...
runcmd:
  ...
  - [sh, -c, "if [ '$node' = r2 ]; then (sleep 80; echo '--- BGP DYNAMIC ROUTE CYCLE ---'; vtysh -c 'conf t' -c 'ip route 198.18.0.0/24 blackhole' -c 'router bgp 65002' -c 'address-family ipv4 unicast' -c 'network 198.18.0.0/24'; echo bgp-add-rc=\$?; sleep 20; vtysh -c 'conf t' -c 'router bgp 65002' -c 'address-family ipv4 unicast' -c 'no network 198.18.0.0/24' -c 'exit-address-family' -c 'no ip route 198.18.0.0/24 blackhole'; echo bgp-withdraw-rc=\$?; ) >/dev/ttyS0 2>&1 & fi"]
