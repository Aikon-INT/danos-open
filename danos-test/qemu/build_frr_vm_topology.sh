...
runcmd:
  ...
  - [sh, -c, "if [ '$node' = r2 ]; then sleep 220; echo '--- BGP DYNAMIC ROUTE CYCLE ---' > /dev/ttyS0; ip route replace blackhole 198.18.0.0/24; vtysh -c 'conf t' -c 'router bgp 65002' -c 'address-family ipv4 unicast' -c 'redistribute static' -c 'exit-address-family' > /dev/ttyS0 2>&1; echo bgp-add-rc=\$? > /dev/ttyS0; vtysh -c 'show bgp ipv4 unicast 198.18.0.0/24' > /dev/ttyS0 2>&1 || true; sleep 20; vtysh -c 'clear bgp 172.31.0.2' > /dev/ttyS0 2>&1; sleep 30; vtysh -c 'conf t' -c 'router bgp 65002' -c 'address-family ipv4 unicast' -c 'no redistribute static' -c 'exit-address-family' > /dev/ttyS0 2>&1; ip route del blackhole 198.18.0.0/24; echo bgp-withdraw-rc=\$? > /dev/ttyS0; fi"]
