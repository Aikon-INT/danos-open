...
runcmd:
  ...
  - [sh, -c, "if [ '$node' = r2 ]; then sleep 80; echo '--- BGP DYNAMIC ROUTE CYCLE ---' > /dev/ttyS0; ip route replace blackhole 198.18.0.0/24; vtysh -c 'conf t' -c 'router bgp 65002' -c 'address-family ipv4 unicast' -c 'network 198.18.0.0/24' -c 'exit-address-family' > /dev/ttyS0 2>&1; echo bgp-add-rc=\$? > /dev/ttyS0; vtysh -c 'show bgp ipv4 unicast 198.18.0.0/24' > /dev/ttyS0 2>&1 || true; sleep 20; vtysh -c 'conf t' -c 'router bgp 65002' -c 'address-family ipv4 unicast' -c 'no network 198.18.0.0/24' -c 'exit-address-family' > /dev/ttyS0 2>&1; ip route del blackhole 198.18.0.0/24; echo bgp-withdraw-rc=\$? > /dev/ttyS0; fi"]
  ...
  - [sh, -c, "if [ '$node' = r1 ]; then sleep 8; echo '--- FRR BGP SUMMARY ---'; vtysh -c 'show bgp summary' || true; echo '--- FRR OSPF NEIGHBORS ---'; vtysh -c 'show ip ospf neighbor' || true; tail -n 80 /var/log/frr/frr.log || true; tail -n 80 /var/log/danos-frr-zapi.log || true; fi"]
EOF
