...
runcmd:
  ...
  - [sh, -c, "if [ '$node' = r2 ]; then sleep 80; echo '--- BGP DYNAMIC ROUTE CYCLE ---' > /dev/ttyS0; vtysh -c 'conf t' -c 'ip route 198.18.0.0/24 blackhole' -c 'router bgp 65002' -c 'address-family ipv4 unicast' -c 'network 198.18.0.0/24' > /dev/ttyS0 2>&1; echo bgp-add-rc=\$? > /dev/ttyS0; vtysh -c 'show bgp ipv4 unicast 198.18.0.0/24' > /dev/ttyS0 2>&1 || true; sleep 20; vtysh -c 'conf t' -c 'router bgp 65002' -c 'address-family ipv4 unicast' -c 'no network 198.18.0.0/24' -c 'exit-address-family' -c 'no ip route 198.18.0.0/24 blackhole' > /dev/ttyS0 2>&1; echo bgp-withdraw-rc=\$? > /dev/ttyS0; fi"]
