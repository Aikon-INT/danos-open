...
runcmd:
  ...
  - [sh, -c, "if [ '$node' = r1 ]; then sleep 120; systemctl restart frr.service; for restart_wait in $(seq 1 30); do test -S /var/run/frr/zserv.api && break; sleep 1; done; if test -S /var/run/frr/zserv.api && vtysh -c 'show bgp summary' | grep -q '172.31.0.3'; then echo FRR-RESTART-TEST PASS > /dev/ttyS0; else echo FRR-RESTART-TEST FAIL > /dev/ttyS0; fi; fi"]
