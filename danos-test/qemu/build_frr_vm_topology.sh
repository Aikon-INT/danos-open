...
runcmd:
  ...
  - [sh, -c, "if [ '$node' = r2 ]; then sleep 420; echo '--- BGP DYNAMIC ROUTE CYCLE ---' > /dev/ttyS0; ip addr replace 198.19.0.1/24 dev ens3; vtysh -c 'conf t' -c 'router bgp 65002' -c 'address-family ipv4 unicast' -c 'redistribute connected' -c 'exit-address-family' > /dev/ttyS0 2>&1; echo bgp-add-rc=\$? > /dev/ttyS0; vtysh -c 'show ip route 198.19.0.0/24' > /dev/ttyS0 2>&1 || true; vtysh -c 'show bgp ipv4 unicast 198.19.0.0/24' > /dev/ttyS0 2>&1 || true; sleep 20; vtysh -c 'clear bgp 172.31.0.2' > /dev/ttyS0 2>&1; sleep 30; vtysh -c 'conf t' -c 'router bgp 65002' -c 'address-family ipv4 unicast' -c 'no redistribute connected' -c 'exit-address-family' > /dev/ttyS0 2>&1; ip addr del 198.19.0.1/24 dev ens3; echo bgp-withdraw-rc=\$? > /dev/ttyS0; fi"]
