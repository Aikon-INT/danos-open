...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --help|-h) echo "usage: $0 [--protocol bgp|ospf] [--messages N]"; exit 0 ;;
...
echo "[INFO] waiting for externally provisioned FRR route add/withdraw events"
set +e
DANOS_ZAPI_PROTOCOL="$ZAPI_PROTOCOL" DANOS_ZAPI_DEBUG=1 \
