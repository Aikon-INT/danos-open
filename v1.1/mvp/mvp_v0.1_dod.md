...
- [ ] **A1 [P0]** `danos-dpa/danos_dpa.h` C ABI 头文件完成，编译通过，无未定义符号
  - 负责：danos-dpa
  - 验收：`gcc -c -I include test_compile.c` 成功

- [ ] **A2 [P0]** `danos-dpa/danos_dpa.proto` Protobuf IDL 完成，`protoc` 编译通过
  - 负责：danos-dpa
  - 验收：`protoc --go_out=. --grpc_out=. danos_dpa.proto` 成功

- [ ] **A3 [P0]** 错误码枚举完整定义（21 个错误码），每个有字符串映射
  - 负责：danos-dpa
  - 验收：`danos_status_str(DANOS_ERR_NO_CAPACITY)` 返回非空字符串

- [ ] **A4 [P0]** 版本协商接口实现：`danos_dpa_get_version()` 返回 `{0, 1, 0}`
  - 负责：danos-dpa
  - 验收：单元测试通过

- [ ] **A5 [P0]** Capability 查询接口实现：`danos_capability_query()` 对每个 ObjectType 返回结果
  - 负责：danos-core
  - 验收：单元测试覆盖所有 ObjectType

- [ ] **A6 [P0]** DPA API conformance 测试套件骨架建立
  - 负责：danos-test/conformance
  - 验收：至少 10 个 conformance 用例可运行

## B. 核心引擎 (danos-core)

- [ ] **B1 [P0]** 对象注册机制实现（Interface / VLAN / VRF / Route / NH / NHGroup / ACL / QoS / BFD）
  - 负责：danos-core/object
  - 验收：每种对象可 create/read/update/delete

- [ ] **B2 [P0]** 状态模型实现：CONFIG → DESIRED → PROGRAMMED → OPER
  - 负责：danos-core/state
  - 验收：单元测试验证四态流转

- [ ] **B3 [P0]** 事务引擎实现：OPEN → PREPARE → VALIDATE → COMMIT → VERIFY → DONE
  - 负责：danos-core/transaction
  - 验收：状态机单元测试覆盖所有路径（含 ABORT/ROLLBACK）

- [ ] **B4 [P0]** 事务并发控制：多读单写，写事务串行
  - 负责：danos-core/transaction
  - 验收：并发测试无数据竞争（ThreadSanitizer 通过）

- [ ] **B5 [P0]** 事务超时机制：prepare 100ms / commit 500ms / verify 1000ms
  - 负责：danos-core/transaction
  - 验收：超时测试返回 `DANOS_ERR_TX_TIMEOUT`

- [ ] **B6 [P1]** WAL（Write-Ahead Log）持久化
  - 负责：danos-core/transaction
  - 验收：崩溃后重启可重放未完成事务

- [ ] **B7 [P0]** 事件总线实现（pub/sub）
  - 负责：danos-core/event
  - 验收：事件订阅/发布单元测试通过

- [ ] **B8 [P0]** Reconciler 骨架：事件驱动 + 周期对账
  - 负责：danos-core/reconciler
  - 验收：模拟 Programmed ≠ Desired 时触发重编程

- [ ] **B9 [P1]** Reconciler 防震荡：5s 窗口内最多 3 次
  - 负责：danos-core/reconciler
  - 验收：震荡测试触发冻结与告警

## C. FRR FIB Adapter (danos-fib)

- [ ] **C1 [P0]** ZAPI 消息解析：ZEBRA_ROUTE_ADD/DELETE、ZEBRA_INTERFACE_ADD/DELETE
  - 负责：danos-fib/zapi
  - 验收：mock ZAPI 消息正确解析

- [ ] **C2 [P0]** ZAPI → DPA 映射表实现（Route / NH / Interface）
  - 负责：danos-fib/mapper
  - 验收：映射单元测试通过

- [ ] **C3 [P0]** FRR zebra 连接管理（连接、断线重连）
...
- [ ] **G1 [P0]** x86_64 构建流水线：每 PR 触发编译 + 单元测试
  - 负责：danos-test/ci
  - 验收：PR 合并门槛：CI 绿灯
...
- [ ] **G5 [P0]** 依赖版本锁定：FRR 10.x / VPP 24.x / DPDK 23.x
  - 负责：danos-build/deps
  - 验收：版本矩阵文档发布
...
- [ ] **H3 [P1]** 多核转发吞吐：4 核 > 5 Mpps
  - 负责：danos-test/perf
  - 验收：TRex 测试报告归档

## I. 文档 (danos-docs)

- [ ] **I1 [P0]** DPA API Specification v0.1 发布
  - 负责：danos-docs/api
  - 验收：本文档系列完成

- [ ] **I2 [P0]** 快速入门教程：5 分钟启动单节点
  - 负责：danos-docs/tutorials
  - 验收：按教程操作可启动

- [ ] **I3 [P1]** 架构决策记录（ADR）初始化
  - 负责：danos-docs/adr
